...
function init-db() {
	...
	docker exec ${POSTGRES_IMAGE} bash -c "psql -h ${POSTGRES_HOST} -U ${POSTGRES_USER} ${OSM_DB} -c 'CREATE EXTENSION postgis;'"
	docker exec ${POSTGRES_IMAGE} bash -c "psql -h ${POSTGRES_HOST} -U ${POSTGRES_USER} ${OSM_DB} -c 'CREATE EXTENSION hstore;'"
}
